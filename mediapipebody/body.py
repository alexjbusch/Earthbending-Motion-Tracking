...
class CaptureThread(threading.Thread):
    ...
    timer = 0.0
    def run(self):
        self.cap1 = cv2.VideoCapture(global_vars.WEBCAM_INDEX1) # sometimes it can take a while for certain video captures 4
        #self.cap2 = cv2.VideoCapture(global_vars.WEBCAM_INDEX2) # sometimes it can take a while for certain video captures 4
        
        if global_vars.USE_CUSTOM_CAM_SETTINGS:
            self.cap1.set(cv2.CAP_PROP_FPS, global_vars.FPS)
            self.cap1.set(cv2.CAP_PROP_FRAME_WIDTH,global_vars.WIDTH)
            self.cap1.set(cv2.CAP_PROP_FRAME_HEIGHT,global_vars.HEIGHT)
            
            #self.cap2.set(cv2.CAP_PROP_FPS, global_vars.FPS)
            #self.cap2.set(cv2.CAP_PROP_FRAME_WIDTH,global_vars.WIDTH)
            #self.cap2.set(cv2.CAP_PROP_FRAME_HEIGHT,global_vars.HEIGHT)

        time.sleep(1)
        
        print("Opened Capture @ %s fps"%str(self.cap1.get(cv2.CAP_PROP_FPS)))
...
class BodyThread(threading.Thread):
    ...
    def run(self):
        ...
        with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity = global_vars.MODEL_COMPLEXITY,static_image_mode = False,enable_segmentation = True) as pose: 
            ...
            while not global_vars.KILL_THREADS and capture.cap1.isOpened():
                ti = time.time()

                # Fetch stuff from the capture thread
                ret = capture.ret1
                image1 = capture.frame1
                #image2 = capture.frame2
                                
                # Image transformations and stuff
                #image = cv2.flip(image, 1)
                image1.flags.writeable = global_vars.DEBUG
                ...
                if global_vars.DEBUG:
                    ...
                    if results2.pose_landmarks:
                        mp_drawing.draw_landmarks(image2, results2.pose_landmarks, mp_pose.POSE_CONNECTIONS, 
                                                mp_drawing.DrawingSpec(color=(0, 100, 255), thickness=2, circle_radius=4),
                                                mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=2, circle_radius=2),
                                                )
                    """                        
                    cv2.imshow('Body Tracking1', image1)                    
                    #cv2.imshow('Body Tracking2', image2)
                    cv2.waitKey(1)
                ...
                if self.pipe != None:
                    ...
                    if results1.pose_world_landmarks:
                        ...
                        image_points = np.float32([[l.x * image1.shape[1], l.y * image1.shape[0]] for l in image_landmarks.landmark])
                        
                        body_world_landmarks_world = self.compute_real_world_landmarks(model_points,image_points,image1.shape)
